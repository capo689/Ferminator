"""Parsers for location strings and compensation data from job HTML."""
...
def parse_locations(raw: str) -> list[str]:
    """Split a raw location string on ; and | delimiters, deduplicate, sort.

    The Greenhouse API uses both semicolons and pipes as separators,
    sometimes mixed in the same string.
    """
...
def parse_compensation(html_content: str) -> dict | None:
    ...
    # Greenhouse's single-job endpoint returns entity-encoded HTML
    # (e.g. "&lt;div&gt;..."). Decode so BeautifulSoup sees real tags.
    if "&lt;" in html_content:
