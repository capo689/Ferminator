...
| Workday | Tenant/site `POST /wday/cxs/{tenant}/{site}/jobs` | None | 67 boards in expanded sweep | Build with contract monitor |
| Breezy | `GET https://{board}.breezy.hr/json` | None | 13 boards in expanded sweep | Build |
| Rippling | Server-rendered `__NEXT_DATA__` board payload | None | 12 boards in expanded sweep | Build with contract monitor |
...
| Jobvite | Documented feed requires a key/secret and supplied public board URLs redirect away | A stable, authorized no-key public contract or approved credentials |
